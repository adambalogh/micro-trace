...
int main(int argc, char *argv[]) {
    ...
    const unsigned short echoServPort =
        atoi(argv[1]);  // First arg:  local port
    ...
    try {
        ...
        for (;;) {
            ...
            if (pthread_create(&threadID, NULL, ThreadMain, (void *)clntSock) !=
                0) {
...
void HandleTCPClient(TCPSocket *sock) {
    ...
    while ((recvMsgSize = sock->recv(echoBuffer, RCVBUFSIZE)) > 0) {
        std::string str(echoBuffer, recvMsgSize);
